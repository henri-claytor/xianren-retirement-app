...
S1 頁面 SHALL 將所有輸入分區包裹於可折疊 Accordion 容器中，每區可獨立展開或收起，並在標題左側顯示 accent color bar。區塊之間依語義分為「流量」（月收入、月支出）與「存量」（現金資產、投資持倉、負債）兩個群組，各群組前方顯示 GroupHeader。
...
#### Scenario: 群組標題渲染
- **WHEN** S1 頁面渲染折疊區塊列表
- **THEN** 「月收入」前顯示「流量 Flow」GroupHeader，「現金資產」前顯示「存量 Stock」GroupHeader
