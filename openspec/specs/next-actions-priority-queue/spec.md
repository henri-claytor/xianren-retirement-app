# next-actions-priority-queue Specification

## Purpose

定義 Dashboard NextActions 推薦清單的計算規則：以純函式 priority queue 規則引擎決定使用者下一步要做的事，涵蓋資料缺口、規劃缺口、強化建議、維運提醒、退休後管理 5 級優先，並支援 visitedTools 追蹤與不再提醒機制。
