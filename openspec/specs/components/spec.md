...
### Sub-nav type 'group' 渲染
Layout.tsx 的 `SubNavItem` 型別 SHALL 支援 `type: 'group'` 變體，以雙列固定結構渲染分組子導覽。
...
- **第一列**：所有 group label 並排，作為 segment tab（等寬、`flex-1`），以 `border-b-[2px]` 作為 active 指示器
...
- **THEN** 第一列 SHALL 顯示所有 group label 為等寬並排按鈕（`flex-1`）
